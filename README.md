...
- **Save / Save As** (`File → Save`, `Ctrl + S` / `File → Save As…`, `Ctrl + Shift + S`) — export the focused image (the one in the viewer, or a single selected thumbnail) to any folder, name, and format. The format list offers only what your Qt build can actually **write**, with a quality slider for the lossy ones (JPEG / WebP). When the viewer holds an unsaved edit, Save writes the edited pixels back over the original (with a confirm) and Save As exports the edited image; with no edit, Save As converts straight from the file on disk. Both run through the task pipeline, so they get the side-by-side conflict prompt if the target already exists.
...
| `C` | Crop — drag a rectangle, then `Enter` to apply / `Esc` to cancel |
| `F11` | Toggle fullscreen |
