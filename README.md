...
- **Thumbnail pipeline** — local SQLite cache, four-worker decode pool, viewport-driven priority queue (top-left → bottom-right), per-session negative caching, automatic background fill of the rest of the folder once the visible items are done. A per-image **Refresh thumbnail** context-menu action force-rebuilds a thumbnail from the file's current bytes — the escape hatch for one cached from a file that was still being written (a long export).
