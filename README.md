...
- **Sorting** — `View → Sort by` orders the file list by name, creation date, or modification date, ascending or descending (two independent, sticky choices). Folders stay grouped ahead of files and `..` stays first regardless. The folder tree and the folder index overlay are always name-sorted.
